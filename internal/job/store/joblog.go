...
type JobLogStore interface {
	// 获取gcron调度器日志的计划任务
	SelectJobLogList(ctx context.Context, jobLog *v1.SysJobLog, opts *api.GetOptions) ([]v1.SysJobLog, error)
	// 查询所有调度任务日志
	SelectJobLogAll(ctx context.Context, opts *api.GetOptions) ([]v1.SysJobLog, error)
	// 通过调度任务日志ID查询调度信息
