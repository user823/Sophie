...
func (r *RedisClient) hashKey(keyname string) string {
	...
	return hash.NewHasher(hash.DefaultHashAlgorithm).HashKey(keyname)
}
...
func (r *RedisClient) IncrememntWithExpire(ctx context.Context, key string, expire int64) int64 {
...
// per 以秒为单位
func (r *RedisClient) SetRollingWindow(ctx context.Context, key string, per int64, val string, pipeline bool) (int, []interface{}) {
	log.Debugf("Incrementing raw key: %s", key)
	if !Connected() {
		return 0, nil
	}

	log.Debugf("keyName is: %s", key)
	now := time.Now()
	...
	pipeFn := func(pipe redis.Pipeliner) error {
		pipe.ZRemRangeByScore(ctx, key, "-inf", strconv.Itoa(int(onePeriodAgo.UnixNano())))
		...
		pipe.ZAdd(ctx, key, element)
		pipe.Expire(ctx, key, time.Duration(per)*time.Second)
		return nil
	}
...
func (r *RedisClient) GetRollingWindow(ctx context.Context, key string, per int64, pipeline bool) (int, []interface{}) {
	if !Connected() {
		return 0, nil
	}
	now := time.Now()
...
func (r *RedisClient) GetAndDeleteSet(ctx context.Context, key string) []interface{} {
	if !Connected() {
		return nil
	}
...
// 添加值到列表中
func (r *RedisClient) AppendToSet(ctx context.Context, key string, value string) {
	if !Connected() {
		return
	}
